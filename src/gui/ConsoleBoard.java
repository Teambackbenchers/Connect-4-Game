...
public int check2InARow(int player) {
	...
	public int evaluate() {
		...
		if(isWin()) {
			...
			else if(getWinner() == Player.two){
				player2Lines = player2Lines + 100;
			}
		}
		
		player1Lines = player1Lines + check3InARow(Player.one) * 10 + check2InARow(Player.one);
		player2Lines = player2Lines + check3InARow(Player.two) * 10 + check2InARow(Player.two);
